...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
    ...
    $prep_stmt = "SELECT artist_id FROM artists WHERE email = ? LIMIT 1";
    ...
    if ($stmt) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            $error_msg .= '<div class="alert alert-danger">A user with this email address already exists.</div>';
//            $stmt->close();
        }
        $stmt->close();
    } else {
...
//        $stmt->close();
    ...
    $prep_stmt = "SELECT artist_id FROM artists WHERE username = ? LIMIT 1";
    $stmt = $mysqli->prepare($prep_stmt);
